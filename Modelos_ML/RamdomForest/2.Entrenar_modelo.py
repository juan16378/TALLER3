...
# Rutas relativas a la ubicación de este script, sin importar desde dónde
# se ejecute (evita crear carpetas nuevas por error)
...
try:
    ...
    df = pd.read_csv(os.path.join(DATA_DIR, "dataset_medico_ampliado.csv"))
    ...
    modelo_path = os.path.join(MODELS_DIR, "modelo_random_forest_ampliado.pkl")
    joblib.dump(model, modelo_path)
    print(f"💾 Modelo guardado en: {modelo_path}")
    
except FileNotFoundError:
    log("ERROR")
    print(f"❌ No se encontró '{os.path.join(DATA_DIR, 'dataset_medico_ampliado.csv')}'")
